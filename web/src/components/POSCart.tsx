...
import { useApp, type Order, type OrderItem } from '../context/AppContext'
...
export function POSCart() {
  ...
  useEffect(() => {
    import('qrcode').then(({ default: QRCode }) => {
      const total = order?.totals.total ?? totals.total
      const payload = JSON.stringify({
        orderId: order?.id ?? previewOrderId,
        total: total.toFixed(2),
        at: order ? order.date : previewDateStr,
        cashier: cashierName,
      })
      QRCode.toDataURL(payload, { margin: 0, width: 120 }).then(setQr).catch(() => setQr(null))
    })
  }, [order?.id, order?.totals.total, order?.date, totals.total, previewOrderId, previewDateStr, cashierName])

  const handleCheckout = () => {
    if (cart.length === 0) return
    const snapshotItems: OrderItem[] = cart.map((line) => {
  ...
  const receiptItems = order?.items ??
    cart.map((line) => {
      ...
      return { productId: line.productId, name: product.name, price: product.price, qty: line.qty, exclusions: line.exclusions }
    })
          ...
          return (
            <div key={line.id} className="flex items-center justify-between gap-2 rounded-md border p-3">
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" size="icon" className="h-12 w-12 text-lg" onClick={() => setQty(line.id, i.qty - 1)}>-</Button>
                <div className="w-8 text-center text-base font-semibold">{i.qty}</div>
                <Button
                  variant="outline"
                  size="icon"
                ...
                {i.product.ingredients && i.product.ingredients.length > 0 && (
                  <Button variant="outline" size="sm" className="h-12 px-4" onClick={() => setCustomizing(line.id)}>Customize</Button>
                )}
                <Button variant="ghost" size="sm" className="h-12 px-3" onClick={() => removeLine(line.id)}>Remove</Button>
              </div>
            </div>
          )
        })}
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <Dialog>
            <DialogTrigger asChild>
              <Button className="flex-1 h-12 text-base" disabled={cart.length === 0}>Checkout</Button>
            </DialogTrigger>
            <DialogContent className="max-h-[90vh] overflow-auto">
              ...
              <div className="print-area receipt mx-auto w-full max-w-[360px] rounded-md bg-white text-slate-900 dark:bg-slate-900 dark:text-slate-100">
                ...
                <div className="grid grid-cols-3 gap-1 px-4 text-xs">
                  ...
                  <div className="text-center">Cashier: {cashierName}</div>
                  <div className="text-right">{orderDateDisplay}</div>
                </div>
                ...
                <div className="px-4 py-2">
                  ...
                  <div className="space-y-1 pt-1 text-xs font-mono">
                    {receiptItems.map((i, idx) => (
                      <div key={`${i.productId}-${idx}`} className="grid grid-cols-[1fr_auto_auto] gap-x-3">
              ...
              <div className="flex flex-col gap-2 pt-3 sm:flex-row">
                <Button className="flex-1 h-12 text-base" onClick={handleCheckout} disabled={cart.length === 0}>Confirm sale</Button>
                <Button variant="outline" className="h-12" onClick={() => window.print()}>Print</Button>
              </div>
            </DialogContent>
          </Dialog>
          <Button variant="secondary" className="flex-1 h-12 text-base" onClick={() => setOrder(null)} disabled={!order}>
      ...
      <Dialog open={!!customizing} onOpenChange={(o) => setCustomizing(o ? customizing : null)}>
        <DialogContent>
          ...
          <div className="space-y-2">
            ...
            <div className="pt-2">
              <Button onClick={() => setCustomizing(null)} className="w-full h-11">Done</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
